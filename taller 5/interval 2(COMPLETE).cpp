...
int main()
{
    struct RB_Trees *succ,*prec,*list = Assing_Nil(),*z;
    int cases,element,LenghtList,queries,operation;
    scanf("%d",&cases);

    for(int index = 1; index <= cases; index++)
    {
        scanf("%d",&LenghtList);
        for(int index2 = 1;  index2 <= LenghtList; index2++)
        {
            scanf("%d",&element);
            z = RB_IterativeTreeSearch(list,element);
            if((z->key) == NIL)
                list = RB_Insert(list,element);
            else
                z->frecuency=(z->frecuency) + 1;
        }

        scanf("%d", &queries);
        for(int index3 = 1; index3 <= queries; index3++)
        {
            scanf("%d %d",&operation,&element);

            if(operation == 1)
            {
                z = RB_TreeSearch(list,element);
                if((z->key) == NIL)
                {
                    list = RB_Insert (list, element);
                    z = RB_IterativeTreeSearch(list, element);
